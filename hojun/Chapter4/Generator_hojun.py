...
class Generator(nn.Module):
    def __init__(self, input_dim, generator_initial_dense_layer_size,
        ...
        self.generator_layer1=nn.Sequential(
            ...
            nn.BatchNorm1d(self.initial_output_size, momentum=self.generator_batch_norm_momentum),
            self.get_activation()
        )
        self.generator_layer2=make_layers(in_channel=self.generator_initial_dense_layer_size[0],
                                         ...
                                         padding=2,
                                         batchnorm_momentum=self.generator_batch_norm_momentum)
        ...
        self.generator_layer4=nn.Sequential(
            ...
            nn.BatchNorm2d(self.generator_conv_channels[2], momentum=self.generator_batch_norm_momentum),
            self.get_activation()
        )
        self.generator_layer5=nn.Sequential(
            ...
            nn.BatchNorm2d(self.generator_conv_channels[3], momentum=self.generator_batch_norm_momentum)
        )
        
    def forward(self, x):
        x=self.generator_layer1(x)
        const=(-1,)
        x=x.view(const+self.generator_initial_dense_layer_size)
...
def make_layers(in_channel, out_channel, kernel_size, padding, batchnorm_momentum):
